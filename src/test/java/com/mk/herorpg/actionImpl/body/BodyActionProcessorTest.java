...
public class BodyActionProcessorTest {
    @Autowired
    @Qualifier("hero")
    private Hero hero;
    ...
    public void excitementProcessing() {
        ...
        processor.process(action, hero);
        assertEquals(-5, hero.getExcitement() - init);
    }
    @Test
    public void moneyProcessing() {
        int init = hero.getMoney();
        processor.process(action, hero);
        assertEquals(-1, hero.getMoney() - init);
    }
    @Test
    public void fitnessProcessing() {
        float init = hero.getFitness();
        processor.process(action, hero);
        assertEquals(0.05f, hero.getFitness() - init, 0.01f);
    }
    @Test
